 ...
 * Initial version created on: 15.01.14 17:01
 */
...
//		print_r($HTTPHeaders);
	 ...
	 * @param bool|string $fmsg the full message of the error
	...
	private function error($c,$fmsg=false,$errorData = Array('no data')){
		...
		$this->response = array (
			...
			'error'	=> array(
				'code'	=> (int)$c,
				'message' => (isset($this->errorMessagesFull[$c])) ? $this->errorMessagesFull[$c] : 'internalError',

				'data'	=> array(
					'request' => (isset($this->request)) ? $this->request : NULL,
					'extension' => (isset($this->extension)) ? $this->extension : NULL,
					'fullMessage' => ($fmsg) ? $this->errorMessagesFull[$fmsg] : $this->errorMessagesFull[$c],
					'debugData' => $errorData
				)
			)
		);
