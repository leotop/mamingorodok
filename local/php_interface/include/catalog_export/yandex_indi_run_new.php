...
if (strlen($strExportErrorMessage)<=0)
{
	...
	while ($obElement = $rsItems -> GetNextElement())
	{
		...
		$arOffersFilter = Array("IBLOCK_ID"=>OFFERS_IBLOCK_ID, "ACTIVE"=>"Y", "PROPERTY_CML2_LINKs"=>$arAcc["ID"]);
		$rsOffers = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arOffersFilter, false, false, array("ID", "IBLOCK_ID", "CATALOG_GROUP_1", "PROPERTY_PICTURE_MAXI", "PROPERTY_COLOR", "PROPERTY_SIZE"));
		while($arOffer = $rsOffers -> GetNext())
		{
			...
			if($arOffer["CATALOG_QUANTITY"]>2 && $offerMinPrice>=1)
			{
			...
			(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;').'utm_source=ya_market&amp;utm_medium=cpc&amp;utm_campaign='.urlencode($strName))."&amp;showOffer=".$arOffer["ID"];
				...
				if (IntVal($arAcc["PROPERTY_PICTURE_MAXI"])>0)
				{
					$picID = IntVal($arAcc["PROPERTY_PICTURE_MAXI"]);
