...
if($STEP == 1) {
	...
	if(CModule::IncludeModule("iblock")) {
		// Get IBlock list
		?>
		<form method="post" name="mail_setup_form" action="<? echo $APPLICATION->GetCurPage() ?>">
		<?= bitrix_sessid_post() ?>
		<?
		...
		<tr>
			...
			<td width="100%" valign="top">
				<select name="IBLOCK_ID" id="IBLOCK_ID" OnChange="document.getElementById('id_ifr').src='/bitrix/tools/catalog_export/mail_util.php?IBLOCK_ID='+this[this.selectedIndex].value;">
			...
			<td width="100%" valign="top">
				...
				<script type="text/javascript">
					...
					function buildMenu() {
						...
						document.getElementById('tree').innerHTML = buffer;
						//BX.adminPanel.modifyFormElements('mail_setup_form');
					}
				...
				<iframe src="/bitrix/tools/catalog_export/mail_util.php?IBLOCK_ID=<?= intval($IBLOCK_ID) ?>" id="id_ifr" name="ifr" style="display:none"></iframe>
				<br><br>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td width="0%" valign="top">
				<font class="text" style="font-size: 20px;">2.4&nbsp;&nbsp;&nbsp;</font>
			</td>
			<td width="100%" valign="top">
				<font class="text"><b>Цена клика</b></font>
				<input name="CBID" value="<?= $arOldSetupVars["CBID"] ?>" type="text" class="price-input">
			</td>
		</tr>
		<tr>
			<td width="0%" valign="top">
					...
					function showDetailPopup() {
						if (null == obDetailWindow) {
							...
							var obDetailWindow = new BX.CAdminDialog({
								'content_url': '/bitrix/tools/catalog_export/mail_detail.php?lang=<?=LANGUAGE_ID?>&bxpublic=Y&IBLOCK=' + s[s.selectedIndex].value,
		...
		<tr>
			...
			<td width="100%" valign="top">
				<font class="text">
					...
					<b><?= COption::GetOptionString("catalog", "export_default_path", "/bitrix/catalog_export/") ?></b><input type="text" name="SETUP_FILE_NAME" value="<? echo (strlen($SETUP_FILE_NAME) > 0)?htmlspecialchars($SETUP_FILE_NAME):"mail.php" ?>" size="50" />
				</font> <br><br>
			</td>
		</tr>
		...
		<tr>
			...
			<td width="100%" valign="top">
				...
				<input type="hidden" name="SETUP_FIELDS_LIST" value="V,IBLOCK_ID,SETUP_SERVER_NAME,SETUP_FILE_NAME,XML_DATA,PRODUCER,PRICE_FROM,PRICE_TO,CBID">
