...
$arYandexFields = array('delivery', 'local_delivery_cost', "typePrefix",'vendor', 'model', 'description', 'manufacturer_warranty', 'country_of_origin', 'store', 'pickup', 'adult', 'author', 'name', 'publisher', 'year', 'ISBN', 'volume', 'part', 'language', 'binding', 'page_extent', 'table_of_contents', 'performed_by', 'performance_type', 'storage', 'format', 'recording_length', 'series', 'artist', 'title', 'year', 'media', 'starring', 'director', 'originalName', 'country', 'sales_notes', 'promo', 'aliases', 'provider', 'tarifplan', 'xCategory', 'additional', 'worldRegion', 'region', 'days', 'dataTour', 'hotel_stars', 'room', 'meal', 'included', 'transport', 'price_min', 'price_max', 'options', 'downloadable', 'param', 'place', 'hall', 'hall_part', 'is_premiere', 'is_kids', 'date');
...
if (strlen($strExportErrorMessage)<=0)
{
	...
	if (!$bAllSections)
	{
		for ($i = 0; $i < count($arSections); $i++)
		{
			...
			while ($ar_res = $db_res->Fetch())
			{
				...
				$arResult["ALL_GROUPS"][$ar_res["ID"]] = $ar_res["NAME"];
			}
	...
	{
		$filter = Array("IBLOCK_ID"=>$IBLOCK_ID, "GLOBAL_ACTIVE"=>"Y"); // "ACTIVE"=>"Y", "IBLOCK_ACTIVE"=>"Y",
		$db_res = CIBlockSection::GetList(array("left_margin"=>"asc"), $filter);
		while ($ar_res = $db_res->Fetch())
		{
			$arAvailGroups[] = array(
	...
	while ($obElement = $db_acc->GetNextElement())
	{
		...
		$rsSubItems = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>OFFERS_IBLOCK_ID, "ACTIVE"=>"Y", "PROPERTY_CML2_LINK"=>$arAcc["ID"]), false, false, array("*"));
		$IDs = array();
		while($ar_fields = $rsSubItems->GetNext())
		{
		...
		else $str_AVAILABLE = ' available="true"';
		...
		if ($MIN_PRICE <= 0) continue;
		$strTmpOff_tmp = "<categoryId>".$arAcc["IBLOCK_SECTION_ID"]."</categoryId>\n";
		...
		foreach ($arYandexFields as $key)
		{
			...
			{
				case 'adult':
					$strTmpOff.= "<adult>false</adult>\n";
				break;
				case 'typePrefix':
					$strTmpOff.= "<typePrefix>".yandex_text2xml($arResult["ALL_GROUPS"][$arAcc["IBLOCK_SECTION_ID"]], true)."</typePrefix>\n";
				break;
				case 'name':
