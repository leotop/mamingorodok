...
if (strlen($strExportErrorMessage)<=0)
{
	...
	file_put_contents("/data/mamingorodok.ru/docs/imp_mail.txt", var_export(array($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME), true));
	...
	else
	{
		...
		if (!@fwrite($fp, '<?if (!isset($_GET["referer1"]) || strlen($_GET["referer1"])<=0) $_GET["referer1"] = "mailt";?>'))
		{
...
if (strlen($strExportErrorMessage)<=0)
{
	...
	@fwrite($fp, "<shopname>".htmlspecialchars($APPLICATION->ConvertCharset(COption::GetOptionString("main", "site_name", ""), LANG_CHARSET, 'windows-1251'))."</shopname>\n");
	...
	else
	{
		...
		while ($arAcc = $db_acc->Fetch())
		{
			if (in_array($arAcc['CURRENCY'], $arCurrencyAllowed))
				$strTmp.= "<currency id=\"".str_replace("RUB", "RUR", $arAcc["CURRENCY"])."\" rate=\"".(CCurrencyRates::ConvertCurrency(1, $arAcc["CURRENCY"], $RUR))."\"/>\n";
		}
	}
	$strTmp.= "</currencies>\n";
	...
	foreach ($arAvailGroups as &$value)
	{
		...
		$arTmpCat[$value["ID"]] = "<category id=\"".$value["ID"]."\"".(IntVal($value["IBLOCK_SECTION_ID"])>0?" parentId=\"".$value["IBLOCK_SECTION_ID"]."\"":" parentId=\"0\"").">".yandex_text2xml($value["NAME"], true)."</category>\n";
	...
	$strOfferFileTmp = $_SERVER["DOCUMENT_ROOT"]."/upload/tmp_mail_".rand(100,999).".tmp";
	$tmpHandle = fopen($strOfferFileTmp, "w");

	$arSelectAddon = array("PROPERTY_CH_PRODUCER.NAME", "PROPERTY_CH_STRANA_1.NAME");
	$arItemsFilter = Array("IBLOCK_ID"=>$IBLOCK_ID, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "!SECTION_ID"=>false, "!PROPERTY_CH_SNYATO"=>2100916, "!PROPERTY_IMPORT2YM"=>2100912);
			...
			while($arOffer = $rsOffers -> GetNext())
			{
				...
				if((empty($PRICE_FROM) || $offerMinPrice >= $PRICE_FROM) && (empty($PRICE_TO) || $offerMinPrice <= $PRICE_TO))
				{
					...
					$strUrl = str_replace(' ', '%20', 'http://'.$ar_iblock['SERVER_NAME'].htmlspecialchars($arAcc["~DETAIL_PAGE_URL"]).
							(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;').'utm_source=mail&amp;utm_medium=cpc&amp;utm_campaign='.urlencode($strName))."&amp;showOffer=".$arOffer["ID"];

					$strResult .= '<offer id="'.$arOffer["ID"].'" available="'.($boolIsAvailable?'true':'false').'" cbid="'.(($CBID>0)?$CBID:30).'">'.$strNL;
