...
<?endif?>
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" class="jqtransform" method="post" enctype="multipart/form-data">
	...
	<?if (is_array($arResult["PROPERTY_LIST"]) && count($arResult["PROPERTY_LIST"] > 0)):?>
			...
			<div>
				...
				switch ($INPUT_TYPE):
					...
					case "T":
						...
						?>
						<textarea width="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]*10?>px" height="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]*10?>px" class="inputtextarea" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
						<?
						}
					break;
					...
					case "N":
						...
						?>
						<input type="text" class="inputtext" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" /><br /><?
		...
		<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
			...
			<label><?=GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT")?><span class="starrequired">*</span>:</label>
			<div>
	...
	<div class="form_footter">
	
		<input type="submit" name="iblock_submit_btn" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
