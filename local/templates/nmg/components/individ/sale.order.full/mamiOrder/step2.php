<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?><?$APPLICATION->IncludeFile($APPLICATION->GetTemplatePath(SITE_TEMPLATE_PATH."/includes/basket_broadcrumb.php"), array("arCrumb"=>3), array("MODE"=>"html") );?>
<?echo ShowError($arResult["ERROR_MESSAGE"]); ?>
<?
function PrintPropsForm($arSource=Array(), $PRINT_TITLE = "", $arParams, $showCaption=true)
{
	?><div class="Form"><?
	if (!empty($arSource))
	{ 
		...
		{
			if($arProperties["GROUP_NAME"] == "Служебные поля") continue;
			
			if($arProperties["SHOW_GROUP_NAME"] == "Y" && $showCaption)
			{ 
				?>
						...
						if($arProperties["ID"] == 10)
						{
							...
							$APPLICATION->IncludeComponent(
								...
								Array(
									...
									"INPUT_NAME" => "ORDER_PROP_10",
									"INPUT_NAME_FINISH" => "",
									"INPUT_VALUE" => (strlen($arProperties["VALUE"])?$arProperties["VALUE"]:$_SESSION["NEW_USER_DATA"]["NEW_DATE"]),
									"INPUT_VALUE_FINISH" => "",
									"SHOW_TIME" => "N",
									"HIDE_TIMEBAR" => "Y"
								),
							...
							//$arDates = getDeliveryDates(5, time());
							
							/*$strSelect = '';
							foreach($arDates as $strDate)
								$strSelect .= '<option'.($arProperties["VALUE"] == $strDate?' selected="selected"':'').' value="'.$strDate.'">'.$strDate.'</option>';
							echo '<select name="ORDER_PROP_10" id="filter_10">'.$strSelect.'</select>';*/
						} else {
							...
							if($arProperties["ID"]==2)
							{
								...
								if(isset($zn[1]))
									$arProperties["VALUE"] = $zn[1];
								else $arProperties["VALUE"] = "";
							}?>
						<input type="text" maxlength="250" class="filter <?if($arProperties["ID"]==3) {?> phone-mask <?}?>" id="filter_<?=$arProperties["ID"]?>" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>">
						<div class="red error error_<?=$arProperties["ID"]?>"></div><?
							if($arProperties["ID"]==3)
							{ ?>
								<div class="clear"></div>
								<div class="inf">Например: +7 123 4567890</div><?
							}
						}
						...
						if ($arParams["USE_AJAX_LOCATIONS"] == "Y"):
							...
							$GLOBALS["APPLICATION"]->IncludeComponent(
								...
								array(
									...
									"LOCATION_VALUE" => ($intCurrentLocation>0?$intCurrentLocation:$value),
								),
								null,
								array('HIDE_ICONS' => 'Y')
							);
...
<table border="0" class="profiler" cellspacing="0" cellpadding="5" width="100%">
	...
	{
		$arResult["PROP_TMP"] = array();
		foreach($arUserProfiles["USER_PROPS_VALUES"] as $arProp)
			$arResult["PROP_TMP"][$arProp["ORDER_PROPS_ID"]] = $arProp;
	?>
		...
		<td>
			<?=$arResult["PROP_TMP"][2]["VALUE"]?>
		</td>
		<td><?=$arResult["PROP_TMP"][4]["VALUE_FORMATED"]?></td>
		<td><?=$arResult["PROP_TMP"][6]["VALUE_FORMATED"]?></td>
		</tr>
	<?	}?>
	<tr>
		<td class="first">
		...
		Создать новый профиль доставки
			...
			echo '<div class="add_prop_container">';
			PrintPropsForm($arResult["PRINT_PROPS_FORM"]["USER_PROPS_N"], GetMessage("SALE_NEW_PROFILE_TITLE"), $arParams, false);
			echo '</div>';
			?>		
		
			<input type="submit" id="btnSendNext" name="contButton"  class="btnSend" value="Продолжить">
			<script type="text/javascript">
				$(document).ready(function() {
					$("#btnSendNext").click(function() {
						$("#orderForm").submit();
						return false;
					}); 
				});
