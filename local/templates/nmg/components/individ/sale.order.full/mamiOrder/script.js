...
function creditClick(intOrderID)
{
	$('#KupiVkreditButton img').click();
	$.ajax({
		type: "POST",
		url: "/system/payment_selected.php",
		data: { ORDER_ID: intOrderID, pay_system: 5 }
	}).done(function() {
...
$(document).ready(function(){
	//$("#NEW_PHONE, input[name='ORDER_PROP_3']").mask("+7 (999) 999-99-99");
     $("#NEW_PHONE, input[name='ORDER_PROP_3']").inputmask("+7(999) 999-99-99");
  ...
  $(".backstep").click(function(){
	var cur = $(this).attr("href");
	$("[name='CurrentStep']").val((cur-1));
	if(cur.length>0)
		$("#orderForm").attr("action", "/basket/order/"+(cur-2)+"/")
	else $("#orderForm").attr("action", "/basket/order/")
	document.order_form.BACK.value='Y'; 
	document.order_form.submit();
	return false;
  });
