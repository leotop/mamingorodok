...
<div class="main_product_headline"><?
    global $arrFilterNew;
    if($arrFilterNew["PROPERTY_NOVINKA_VALUE"] == "Да"){?>
       <p>Новинки</p>                        
    <?}else if($arrFilterNew["PROPERTY_KHIT_PRODAZH_VALUE"] == "Да"){?>
       <p>Хиты продаж</p>  
    <?}
	if(false)
	{?>
	...
	foreach($arResult["ROWS"] as $arItems)
	{
		...
		<!--<span class="fat top_price"><?=CurrencyFormat($arElement["PROPERTIES"]['PRICE']["VALUE"], "RUB")?></span> --> 
                <?
                    ...
                    {?>
                        <span class="fat top_price"><?=number_format($price, 0, '.', ' ' );?> <div class="rub_none">руб.</div><span class="rouble">a</span></span><?
                    }?>
                            <?
