            ...
            <table class="catalogLevel1"><?
                    foreach ($arResultTMP as $arSec) {
                        if ($intCnt > 0 && $intCnt%4 == 0) {
                            echo '<tr>'.implode(" ", $arTd["TITLE"]).'</tr><tr>'.implode(" ", $arTd["PICTURE"]).'</tr><tr>'.implode(" ", $arTd["LINKS"])
                            .'</tr><tr class="catalogLevel1Spacer"><td colspan="100%"></td></tr>';
                            $arTd = array();
                            $intCnt = 0;
                        }
                        ...
                        $arTd["PICTURE"][] = '<td class="picture'.($intCnt==3?' last':'').'">'.($arSec["PICTURE"]>0?'<a href="'.$arSec["SECTION_PAGE_URL"].'"><img src="'
                            .CFile::GetPath($arSec["PICTURE"]).'" alt="'.$arSec["NAME"].'"></a>':' ').'</td>';
                        $strLinks = '';
