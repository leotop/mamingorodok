			...
			foreach($arResult["ORDER_BY_STATUS"] as $key => $val)
			{
				...
				foreach($val as $vval)
				{
					...
					if($bShowStatus)
					{
						...
						<small><?=$arResult["INFO"]["STATUS"][$key]["DESCRIPTION"] ?></small><br><br>
					<?
					}
					$bShowStatus = false;
					?>
					<table class="sale_personal_order_list">
						...
						<tr>
							<td align="right" class="order_actions">
								<?
								if($vval["ORDER"]["STATUS_ID"] == "A") {
									if ($vval["ORDER"]["PAY_SYSTEM"]["PSA_NEW_WINDOW"] == "Y")
									{ ?>
									<a href="<?=$vval["ORDER"]["PAY_SYSTEM"]["PSA_ACTION_FILE"]?>" target="_blank">Оплатить заказ</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <?
									} else {
									$ORDER_ID = $vval["ORDER"]["ID"];
									include($vval["ORDER"]["PAY_SYSTEM"]["PSA_ACTION_FILE"]);
									}
								}?>
