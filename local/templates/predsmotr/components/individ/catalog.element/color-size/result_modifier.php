...
while($obEl = $dbEl->GetNext())    
{
    ...
    if (!in_array($obEl["PROPERTY_COLOR_CODE_VALUE"], $arResult["LINKED_COLORS"]) && $obEl["QUANTITY"]>0 && $obEl["PRICE"]>0)
    {
        $arResult["LINKED_COLORS"][] = $obEl["PROPERTY_COLOR_CODE_VALUE"];
        $arResult["LINKED_COLORS_ITEMS"][] = $obEl;
    }
	//echo $obEl["PROPERTY_SIZE_VALUE"]."---<br>";
    if (!in_array($obEl["PROPERTY_SIZE_VALUE"], $arResult["LINKED_SIZES"]) && $obEl["QUANTITY"]>0 && $obEl["PRICE"]>0)
    {
...
SortArray($arResult["LINKED_ITEMS"],"PRICE");
SortArray($arResult["LINKED_COLORS_ITEMS"],"PRICE");
SortArray($arResult["LINKED_SIZES_ITEMS"],"PRICE");
//die(print_R($mas));
//$arResult["LINKED_ITEMS"] = $mas;
//print_r($arResult["LINKED_ITEMS"]);
//,"PRICE");
//die(print_r($arResult["LINKED_ITEMS"]));
