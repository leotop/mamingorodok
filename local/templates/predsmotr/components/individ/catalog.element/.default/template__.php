...
<div id="DetailPhotoChoose">    
    ...
    <div id="PictureBlock" clearfix>
		...
		<a href="/inc/picture_view.php?ELEMENT_ID=<?=$arResult["ID"]?>&img_id=<?=$arResult["DETAIL_PICTURE"]["ID"]?>" class="fancybox jqzoomix" ><img alt="<?=$arResult["DETAIL_PICTURE"]["SRC"]?>" src="<?=$file?>"></a>
		<?}else{?>
		<a href="#" class="fancybox jqzoomix"><img src=""></a>
		<?}?>
		<?endif?>
		</div>
		...
		///print_R($file);
		?>
        <div class="DPictureList<?if ((count($arResult["PROPERTIES"]["IMG_BIG"]["VALUE"]) + 1 ) < 6):?>  DPictureListMargin<?endif;?><?if ((count($arResult["PROPERTIES"]["IMG_BIG"]["VALUE"]) + 1 ) > 5):?> DPictureListCarousel<?endif;?>">
            <ul>
            ...
            <?foreach($arResult["PROPERTIES"]["IMG_BIG"]["VALUE"] as $img_id):?>
                ...
                <li>
					...
					<?$file = CFile::ResizeImageGet($img_id,  array("width"=>256, "height"=>256),BX_RESIZE_IMAGE_PROPORTIONAL)?>
                    <span class="midi-picture-src" style="display:none;"><?=$file["src"]?></span>
                    ...
                    <?foreach($arResult["LINKED_COLORS_ITEMS"] as $arColorItem):?>
                        ...
                        <div class="item color-id-<?=$arColorItem["PROPERTY_COLOR_CODE_VALUE"]?>">
                            <label for="color_<?=$arColorItem["PROPERTY_COLOR_CODE_VALUE"]?>">
								...
								<?$file = CFile::ResizeImageGet($arColorItem["PROPERTY_PICTURE_MINI_VALUE"],  array("width"=>32, "height"=>32),BX_RESIZE_IMAGE_PROPORTIONAL)?>
                                <img src="<?=$file["src"]?>" />
