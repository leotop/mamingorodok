...
<?if($arResult["PROPERTIES"]["vote_count"]["VALUE"]):?>
        <div class="vote-result none"><?=GetMessage("T_IBLOCK_VOTE_RESULTS", array("#VOTES#"=>$arResult["PROPERTIES"]["vote_count"]["VALUE"] , "#RATING#"=>$DISPLAY_VALUE))?></div>
    <?else:?>
        <div class="vote-result none"><?=GetMessage("T_IBLOCK_VOTE_NO_RESULTS")?></div>
    <?endif?>
...
<script type="text/javascript">
    $(document).ready(function(){
        simple_tooltip($(".vote-div"), 'tool', <?=$arResult["ID"]?>, 'right');
        $(".star-active").click(function(){
               $(".tool").hide();
        });
    })
</script>
