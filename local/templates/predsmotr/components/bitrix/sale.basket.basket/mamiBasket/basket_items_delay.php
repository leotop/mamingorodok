...
<table class="sale_basket_basket data-table">
	...
	foreach($arResult["ITEMS"]["DelDelCanBuy"] as $arBasketItems)
	{
		...
		<tr>
			...
			<?endif;?>
			<?if (in_array("WEIGHT", $arParams["COLUMNS_LIST"])):?>
				<td align="right"><?echo $arBasketItems["WEIGHT"] ?> <?=(strlen($arResult["WEIGHT_UNIT"]) > 0 ? $arResult["WEIGHT_UNIT"] : GetMessage("SALE_WEIGHT_G"))?></td>
			<?endif;?>
		</tr>
		<?
	}
...
<div width="30%">
	...
	<input type="hidden" value="<?= GetMessage("SALE_REFRESH")?>" name="BasketRefresh">
	<br />
