<?
    ...
    if(!$isSearch)
    {
        if(preg_match("#/catalog/brend-.*/#", $APPLICATION->GetCurDir()) || preg_match("#/test/brend-.*/#", $APPLICATION->GetCurDir()))
            ...
            foreach($arResult["ROWS"] as $arRow)
            {
                ...
                <li id="<?=$strMainID?>">
                    <div class="catalog_bg stock-item"><?   
                            ...
                            //arshow($arFileTmp);
                        ?>
                        <div class="photo"><p><i title="<?=$arElement['DETAIL_PAGE_URL']?>"><img src="<?=$arFileTmp["src"]?>" alt="<?=(empty($arElement["PROPERTIES"]["SEO_H1"]["~VALUE"])?$arElement["NAME"]:$arElement["PROPERTIES"]["SEO_H1"]["~VALUE"])?>"></i><span>&nbsp;</span></p></div>
                        <?
                            $strAddon = "";
                            if(!$isSearch) {
                        ...
                        <div class="price">
                            <?
                               ...
                               // arshow($arResult["COUNT_SKLAD"]);
                                if(($arElement["PROPERTIES"]["CH_SNYATO"]["VALUE_ENUM_ID"] == 2100916) || $price == 0/* || $arElement["COUNT_SKLAD"] <=2*/) //проверка цены если больше 0
                                    echo '<span class="currency" style="width: 100%;font-size:12px">Нет в наличие</span>';
                                else 
                                {?>
