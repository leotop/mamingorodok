...
{
	$arErr = array();
	if(strlen($_REQUEST["qoName"])<=2) $arErr[] = 'Не заполнено поле "Имя"';
	if(strlen($_REQUEST["qoPhone"])<=2) $arErr[] = 'Не заполнено поле "Телефон"';
	...
	if(count($arErr)>0)
		...
		if(!$USER -> IsAuthorized())
		{
			...
			$rsRegister = $USER->Register($_REQUEST["qoEmail"], $_REQUEST["qoName"], "", $strDefPass, $strDefPass, $_REQUEST["qoEmail"]);
