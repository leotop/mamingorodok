...
$(function() {
    ...
    $('.jqtransform-cover form').jqTransform();
	});
...
//	$('.choose  a').click(function() {return false;});
    ...
    $("#CatalogCenterColumn").delegate('.add-to-compare-list-ajax',"click",function(){
    ...
    $("#CompareList .add").click(function(){
        var product_id = parseInt($('#product-id').html());
        if (product_id > 0)
        {
            $.get('/add-to-compare-list.php', { 'id': product_id, 'action': 'ADD_TO_COMPARE_LIST' }, function(change_data){
                $('#CompareList').html('<div class="DIcon" ></div> <a href="/catalog/compare/">В список сравнения</a><div class="clear"></div>');
            })
        }
