...
<?
    ...
    else
    {
        ...
        <?if($arResult["Post"]["TYPE"]==BLOG_TYPE):?>
			...
			<a href="/community/user/<?=$arResult["arUser"]["ID"]?>/"><?=$arResult["LOOK_NAME"]?></a> <span class="rating">Рейтинг:</span> <?if($arResult["arUser"]["UF_USER_RATING"]>0):?>+<?endif;?><?=$arResult["arUser"]["UF_USER_RATING"]?> <span class="rating">Статус:</span> <?=GetStatusByRatingValue($arResult["arUser"]["UF_USER_RATING"], $arStatusList);?>
                    ...
                    <noindex>
                        <?
                            ...
                            foreach($arResult["Category"] as $v)
                            {
                                if($i!=0)
                                    echo ",";
                            ?> <a href="/community/group/<?=$arResult["Blog"]["URL"]?>/category/<?=$v["NAME"]?>/" class="grey" rel="nofollow"><?=$v["NAME"]?></a><?
                                $i++;
                            }
                        ?>
                    </noindex>
