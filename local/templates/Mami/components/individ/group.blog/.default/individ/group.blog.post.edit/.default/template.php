...
<div class="panelRight"></div>
		...
		<div class="blog-post-fields blog-edit-fields">
			...
			<div class="rubricks">
				...
				$j=0;
				$h = 0;
				$tags = explode(", ",$arResult["PostToShow"]["CategoryText"]);
				//print_R($tags);
				foreach($arResult["ITEMS"] as $k=>$v){
					if($j==0) {echo "<tr>";}
					?>
						<td><input type="checkbox" id='rub<?=$h?>' class="rubric_ch" name="rubric[]" value="<?=$v?>"<?if(in_array($v,$tags)):?> checked<?endif;?>><label for='rub<?=$h?>'><?=$v?></label>
						<div class="clear"></div>
						</td>
					<?
					$h++;
