...
<?if(count($arResult["ROWS"])>0):?>
    <div class="goods">
    <h2>Популярные товары <a href="/catalog/title/popular/">Показать все</a></h2>
    <div class="item_list"> 
<?$first = true;?>
        ...
        <div class="item <?=($first ? 'first' : ($last ? 'last' : ''))?>"><div class="align_center_to_left">
            ...
            <?else:?>
                <div class="align_center_to_right">
					...
					<div class="Price"><span class="Currency">от</span> <?=number_format($arElement["PROPERTIES"]['PRICE']["VALUE"], 0, '.', ' ')?> <span class="Currency">р</span></div>
					<?endif?>
				</div>
            <?endif;?>
            <?if($InBasket):?>
