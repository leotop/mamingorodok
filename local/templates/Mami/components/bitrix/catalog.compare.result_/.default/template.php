...
foreach($arResult["ITEMS"] as $k => $arItem)
{
	...
	if(!$USER->IsAuthorized()):
		$arResult["ITEMS"][$k]["ADD_TO_WISH"] = '<div id="BabyList"><div class="BabyListCenter"><div class="DIcon" ></div> <a class="showpUp greydot heartDICON" href="#messageNoUser1">В список малыша</a></div></div>';
	else:	  
		...
		if($obEl = $dbEl->GetNext())
			$arResult["ITEMS"][$k]["ADD_TO_WISH"] = '<div class="action prod_id'.$arItem["ID"].'" id="BabyList"><div class="DIcon" ></div> <a href="/community/user/'.$user_id.'/">Уже в списке малыша</a><div class="clear"></div></div>';
		else
			$arResult["ITEMS"][$k]["ADD_TO_WISH"] = '<div class="action prod_id'.$arItem["ID"].'" id="BabyList"><div class="DIcon" ></div> <a class="add" href="#">В список малыша</a><div class="clear"></div></div>';
	endif;
}
...
<table class="CompareCatalog" width="100%">
    ...
    <tr class="Prices">
        <td class="first">&nbsp;</td>
        <?foreach($arResult["ITEMS"] as $arItem):?>
            <td><?=$arItem["PROPERTIES"]["PRICE"]["VALUE"]?> <span>руб.</span></td>
        <?endforeach?>
    </tr>
    <tr class="Basket">
        <td class="first">&nbsp;</td>
        <?foreach($arResult["ITEMS"] as $arItem):?>
            <td><a href="/select-color-and-size.php?id=<?=$arItem["ID"]?>" class="ToBasket-compare add-to-basket"></a></td>
        <?endforeach?>
    </tr>
