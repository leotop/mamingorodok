...
else
{
	...
	<?if ($arResult["Events"]):?>
				...
				<tr>
					<td valign="top" width="10%" nowrap>
						<?if ($event["EventType"] == "FriendRequest"):?>
							<center><?= $event["Event"]["USER_PERSONAL_PHOTO_IMG"]; ?></center><br>
							<?
								$name = "";
								if(!empty($event["Event"]["USER_NAME"])){
									$name = $event["Event"]["USER_NAME"];
								}
								
								if(!empty($event["Event"]["USER_LAST_NAME"])){
									if(!empty($name))
										$name .= " ".$event["Event"]["USER_LAST_NAME"];
									else
										$name = $event["Event"]["USER_LAST_NAME"];
								}
								
								if(empty($name)){
									$name = $event["Event"]["USER_LOGIN"];
								}
								?>
								<a href="/community/user/<?=$event["Event"]["USER_ID"]?>/"><?=$name?></a>
								<?
		...
		<br /><br />
	<?else:?>	
		<div class="notetext">У Вас больше нет сообщений.</div>
	<?endif;?>
	<?
}
?>