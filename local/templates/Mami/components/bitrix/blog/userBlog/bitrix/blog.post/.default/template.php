...
else
{
	...
	if(!empty($arResult["Post"])>0)
	{
		...
		<div class="clear"></div>
			...
			<a href="/community/user/<?=$USER->GetID()?>/"><?=$arResult["LOOK_NAME"]?></a> <span class="rating">Рейтинг</span> <?if($arResult["RATING"]>0):?>+<?endif;?><?=$arResult["RATING"]?> <span class="rating">Статус:</span> <?=GetStatusByRatingValue($arResult["RATING"], $arStatusList);?></td>
		<div class="clear"></div>
		<div class="text2">
		...
		<?elseif($arResult["Post"]["TYPE"]==ADD_REPORT_TYPE):?>	
			<?if($arResult["Blog"]["OWNER_ID"]!=$USER->GetID()):?>
				...
				<div class="status">
				<?//echo $arResult["Post"]["USER_GENDER"];?>
				<div><?if($arResult["Post"]["USER_GENDER"]=="M"):?>Запросил<?else:?>Запросила<?endif?> отзыв у <a href="/comunity/blog/<?=$arResult["Post"]["REPORT_USER_BLOG"]?>/"><?=$arResult["Post"]["REPORT_USER"]?></a> на товар</div>
				<div class="tvr">
				<table>
				<tbody>
				<tr>
			...
			<?else:?>
				...
				<?//print_R($CurPost)?>
				<div class="clear"></div>
				<div class="st">Я <?if($CurPost["USER_GENDER"]=="M"):?>запросил<?else:?>запросила<?endif?> отзыв у <a href="/comunity/blog/<?=$arResult["Post"]["REPORT_USER_BLOG"]?>/"><?=$arResult["Post"]["REPORT_USER"]?></a> на товар	
				</div>
				<?if($arResult["Post"]["PRODUCT"]["PREVIEW_PICTURE"]>0):?>
					<?=ShowImage($arResult["Post"]["PRODUCT"]["PREVIEW_PICTURE"],60,60,'class="stfoto tovarfoto"');?>
				<?endif;?>
				<div class="tovarName"><a style="margin-top:15px;" href="<?=$arResult["Post"]["PRODUCT"]["DETAIL_PAGE_URL"]?>"><?=$arResult["Post"]["PRODUCT"]["NAME"]?></a></div>
				<div class="clear"></div>
			<?endif;?>
		<?elseif($arResult["Post"]["TYPE"]==CERTIFICATE_TYPE):?>	
			...
			<?if($arResult["Blog"]["OWNER_ID"]!=$USER->GetID()):?>
				...
				<?//echo $arResult["Post"]["USER_GENDER"];?>
				<div><?if($arResult["Post"]["USER_GENDER"]=="M"):?>Подарил<?elseif($arResult["Post"]["USER_GENDER"]=="F"):?>Подарила<?else:?>Подарил(а)<?endif?> пользователю <a href="/comunity/user/<?=$arResult["Post"]["USER_TO"]["ID"]?>/"><?=$name?></a> сертификат </div>
				<div class="tvr">
				<table>
				<tbody>
				<tr>
			...
			<?else:?>
				...
				<?//print_R($CurPost)?>
				<div class="clear"></div>
				<div>Я <?if($arResult["Post"]["USER_GENDER"]=="M"):?>подарил<?elseif($arResult["Post"]["USER_GENDER"]=="F"):?>Подарила<?else:?>Подарил(а)<?endif?> пользователю <a href="/comunity/user/<?=$arResult["Post"]["USER_TO"]["ID"]?>/"><?=$name?></a> сертификат </div>	
				</div>
				<?if($arResult["Post"]["CERTIFICATE"]["PREVIEW_PICTURE"]>0):?>
