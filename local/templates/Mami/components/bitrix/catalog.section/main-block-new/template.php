    ...
    <?foreach($arItems as $arElement):?>
        ...
        <div class="item <?=($first ? 'first' : ($last ? 'last' : ''))?>"><div class="align_center_to_left">
            ...
            <?if($arElement["PROPERTIES"]['OLD_PRICE']["VALUE"] > 0):?>
                ...
                <div class="align_center_to_right">
				<?if($arElement["PROPERTIES"]['PRICE']["VALUE"]>0):?>
				<div class="NewPrice"> <span class="Currency">&nbsp;</span> <?=number_format($arElement["PROPERTIES"]['PRICE']["VALUE"], 0, '.', ' ')?> <span class="Currency">р</span></div>
				<?else:?>
				&nbsp;
				<?endif?>
				</div>
            <?else:?>
				...
				<?if($arElement["PROPERTIES"]['PRICE']["VALUE"]>0):?>
				 <span class="Currency">&nbsp;</span> <?=number_format($arElement["PROPERTIES"]['PRICE']["VALUE"], 0, '.', ' ')?> <span class="Currency">р</span>
				<?else:?>
				&nbsp;
				<?endif?>
				</div></div>
