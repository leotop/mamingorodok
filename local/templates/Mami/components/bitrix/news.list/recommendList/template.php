			...
			<?if($i==0):?>
				<div class="preview-preview" id='tovar_<?=$tovar["ID"]?>_<?=$j?>'>
					...
					<div class="right-right">
						...
						<?if($tovar["HAVEBUY"]=="Y"):?>
							<a href="/select-color-and-size.php?id=<?=$tovar["ID"]?>" class="add-to-basket"></a>
						<?else:?>
			...
			<?else:?>
				 <div class="item-item item_item_<?=$tovar["ID"]?>_<?=$j?><?if ($i==count($arItem["TOVAR"])-1):?> last<?endif?>">
					...
					<div class="right-right">
						...
						<?if($tovar["HAVEBUY"]=="Y"):?>
							<a class="add-to-basket" href="/select-color-and-size.php?id=<?=$tovar["ID"]?>"></a> 
						<?else:?>
							<a class="add-to-basket-none" href="#" onclick="return false;"></a> 
						<?endif;?>
