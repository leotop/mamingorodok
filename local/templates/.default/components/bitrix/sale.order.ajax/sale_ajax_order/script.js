BX.saleOrderAjax = { // bad solution, actually, a singleton at the page
 ...
 $(window).load(function(){       // активирует первый адрес пользователя если он существует
       $('.table_adres li:first-child input').click();  
 });  
function clickAdress(id_adress){   
     $('#ID_PROFILE_ID_'+id_adress).click(); 
     console.log('#ID_PROFILE_ID_'+id_adress);
 return true;
  }