<?
    ...
    if ($USER->IsAuthorized()){
        ...
        if(!empty($arResult["ITEMS"])) {
            $arTmpOffers = array();
            $rsI = CIBlockElement::GetList(array(), array("ACTIVE" => "Y",    "IBLOCK_ID" => 3, "PROPERTY_MAIN_PRODUCT" => array_keys($arResult["ITEMS"])), false, false, array("ID", "IBLOCK_ID", "CATALOG_GROUP_1", "PROPERTY_MAIN_PRODUCT"));
            while($arI = $rsI->GetNext())
                $arTmpOffers[$arI["PROPERTY_MAIN_PRODUCT_VALUE"]][] = $arI["CATALOG_PRICE_3"];
