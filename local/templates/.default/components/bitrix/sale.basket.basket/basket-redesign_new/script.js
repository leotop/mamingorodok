...
function updateBasketTable(basketItemId, res)
{
    ...
    if (!!res.BASKET_DATA)
    {   */
    //Script for update basket items
    $("#bx_ordercart_order_table_container").load("/basket/ #bx_ordercart_order_table_container > *", function(){
        animateCalcCar();
    });
    //    $("#basket_items_delayed").load("/basket/ #basket_items_delayed > *"); 
...
//Animate road line delivery
$(function() {
    animateCalcCar();
});

function animateCalcCar() {
    var allWidth=$(".bottom-road").width();
    var width = '300px';
    var car_margin = '250px';

    var priceDelivery;
    var sumFreeDelivery;
    var sumBasket = parseInt($("#allSum_FORMATED").html());
    if (sumBasket>5000) {
        priceDelivery=0;
        width = allWidth;
    } else {
        priceDelivery=5000-sumBasket;
        widthProc=sumBasket/(5000/100)
        width = (allWidth/100)*widthProc;  
    }
    if(priceDelivery == 0){
        priceDelivery = '<span class="free_del_basket">¡≈—œÀ¿“ÕŒ</span><span class="super_message_basket"> —”œ≈–!</span>';
        $(".free-delivery .rouble").hide();
        $(".free-delivery > div > p").html('Ã˚ ‰ÓÒÚ‡‚ËÏ ‚‡Ï Á‡Í‡Á:')
    }
    $(".summ-free-delivery").html(priceDelivery);
    car_margin = width - 50;
    width = width+"px";
    ...
    $("#car").animate({
        marginLeft: car_margin 
        }, 1000
    );
}


//Function for switch auth, reg
$(function() {
    $(".bttn").click(function () {
        $(".bttn").removeClass("current");
        $(this).addClass("current");        
        var idElem = $(this).prop("id");
        var splId = idElem.split("-");
        var idForm = '.'+splId[0]+'-form'
        $('.bForm').hide();
        $(idForm).show();
    });
});

//Function checked on registration
function checkFormBox() {
    var check = $("#reg-check").prop("checked");
    if (check==true) {
        $( "#registerForm" ).submit(); 
    } 
}
...
//Set quantity on delayed items
function delay_upd(id,sign,quantity, price){
    $.post("/ajax/ajax_delay.php",{id:id,sign:sign},function(data){
        $("#basket_items_delayed").load("/basket/ #basket_items_delayed > *");
    })
