...
if (!empty($arResult['ITEMS']))
{
	...
	<div class="crumbs_black_big element"><? echo GetMessage('CVP_TPL_MESS_RCM') ?></div>
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
        ...
        $price=GetOfferMinPrice(2,$arItem["ID"]);
	...
	{
		?>
		<div class="bx_catalog_item_title link">
            <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="bx_rcm_view_link" data-product-id="<?=$arItem['ID']?>" title="<? echo $arItem['NAME']; ?>"><?=smart_trim($arItem['NAME'], 45)?></a>
        </div>
	<?
	}?>
	<div class="bx_catalog_item_price">
            ...
            <?//arshow($arItem['MIN_PRICE'])?> 
            <?if(!empty($price)){ ?>
            <span class="acess_price">Цена:&nbsp;<?=number_format($price, 0, '.', ' ' )?><div class="rub_none">руб.</div><span class="rub"> a</span><?
            }else{?> 
                <span class="acess_price_of">нет в наличии</span><?
            };?>
